...
<img src="./_images/1_tim_berners_lee.PNG" width=100%>

모든 발명품이 특정한 문제로부터 시작하였듯이, 월드 와이드 웹도 당시 팀이 가지고 있던 어떠한 불만으로부터 출발했다. 당시 CERN이라는 연구소에서 일하고 있던 그는 연구소 내에서 정보를 찾는 방식에 대해 큰 문제의식을 느끼고 있었다. 그 당시에 연구소 내에서 정보는 수많은 컴퓨터들에 분산되어 계층적인 폴더 구조에 저장되는 방식이었는데, 이 방식이 __필요한 정보를 찾는데 있어 굉장히 불편 했기 때문__ 이다.
