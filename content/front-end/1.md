...
이처럼 사람은 연상을 통해 사고가 자연스럽게 하나에서 또 다른 것으로 즉시 이어지는데 반해, 당시 CERN의 정보 시스템은 순차적이고 계층적인 구조였기 때문에 __파일을 찾는데 많은 시간이 걸렸기 때문에 굉장히 비효율적__ 이었다. 위의 예시처럼 문서 A 하나를 읽는데만 5 문서가 더 필요했는데, 당시 CERN의 정보 시스템에서는 하나의 파일을 찾기 위해 어느 컴퓨터에 관련 내용이 저장되어있는지부터, 해당 컴퓨터 시스템 내의 어느 폴더에 파일이 위치했는지까지도 찾아야했기 때문에 굉장히 오랜 시간이 걸렸던 것이다. 즉, 당시 CERN의 정보 시스템은 __연상적으로 정보를 참조하기에 적절하지 않았던 것__ 이다.
