...
<img src="./_images/1_web_tim_berners_lee1.PNG" width=100%>

모든 발명품이 특정한 문제로부터 시작하였듯이, 월드 와이드 웹도 당시 팀이 가지고 있던 어떤 불만에서부터 출발했다. 당시 CERN이라는 연구소에서 일하고 있던 그는 연구소 내에서 정보를 찾는 방식에 대해 큰 문제의식을 느끼고 있었다. 그 당시에 연구소 내에서 정보는 수많은 컴퓨터들에 각각 분산되어 계층적인 폴더 구조에 저장되는 방식이었는데, 이 방식이 __필요한 정보를 찾는데 있어 굉장히 불편 했기 때문__ 이다.

예를 들기 위해, 위에 소개된 그림처럼 팀이 __`문서 A를 읽는 과정`__ 을 자세히 살펴보자. 
- 문서 A를 읽는 와중에 __`단어 B2`__ 를 처음 보게 된다. 
    - B2이 무엇인지 알기 위해, B2과 __관련된 파일을 CERN의 정보 시스템에서 찾는다__.
    - 정보가 각 컴퓨터에 분산되어 있기 때문에 먼저 __어떤 컴퓨터에 B1이 저장되어 있는지__ 를 찾아야 한다.
    - 해당 컴퓨터의 __파일 시스템에서 관련 파일__ 을 찾아, B2이 무엇인지 읽고 이해한다.
- 다시 문서 A의 나머지 부분을 읽는데, __`B1 실험 결과`__ 가 문서 A의 내용과 긴밀한 관련이 있음을 깨닫게 된다.
    - B1 실험결과를 명확하게 확인하기 위해, B1와 관련된 파일을 __또 CERN의 정보 시스템에서 찾아 읽는다__ (위의 과정 반복)
- B1 실험 결과에 대해 읽다가 문득, 왜 하필이면 __`C2 이론`__ 을 적용하였는지가 궁금해졌다.
    - C2 이론에 대해 더 자세히 알기 위해, C2과 관련된 파일을 __또 다시 CERN의 정보 시스템에서 찾아 읽는다__ (또 과정 반복)
- C2 이론 문서를 읽으면서 이를 쓴 __`연구원 B3`__, 그리고 그의 또 다른 __`이론 C1`__ 에 대해 더 자세히 알고 싶어졌다.
    - B3와 C1과 관련된 __파일들을 CERN의 정보 시스템에서 각각 찾아 읽는다__ (이걸로 5번째 반복)
- 각각 필요한 정보들을 얻었으면, __다시 `문서 A` 로 돌아와__ 마저 글을 읽는다.

이처럼 사람은 연상을 통해 사고가 자연스럽게 하나에서 또 다른 것으로 이어지는데 반해, __당시 CERN의 정보 시스템은__ 순차적인 또는 계층적인 구조였기 때문에 __연상적으로 정보를 참조하기에 적절하지 않았던 것__ 이다. 

무엇보다도, 가장 큰 문제는 새로운 정보를 추가하거나 이미 있는 정보들을 수정해야 할 경우, 시스템 전체가 구조적인 개정이 필요할 수도 있다는 점이었다. 이러한 점들에서 팀은 사람의 자연스러운 연상 과정을 따라 정보를 체계화하는 새로운 정보 시스템이 필요하다고 주장하며,
- 각 정보마다 고유한 주소인 __URL__ 을 표시,
- 정보를 서로 유기적으로 연결하는 __하이퍼링크 (Hyperlink)__ 개념을 도입,
- 하이퍼링크를 문서파일에 삽입할 수 있는 __HTML (Hypertext Markup Language)__ 이라는 언어를 창조,
- HTML로 쓰여진 하이퍼텍스트 파일을 주고 받을 수 있는 전송 양식인 __HTTP (Hypertext Transfer Protocol)__ 를 정의,

그리고 이 모두를 기반으로 하여 결국, 연상적으로 정보를 참조할 수 있는 정보 시스템인 __월드 와이드 웹 (World Wide Web)__ 을 만들게 되었다.
