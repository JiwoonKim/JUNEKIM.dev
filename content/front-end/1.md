...
모든 발명품이 특정한 문제로부터 시작하였듯이, 월드 와이드 웹도 당시 팀이 가지고 있던 어떠한 불만으로부터 출발했다. 당시 CERN이라는 연구소에서 일하고 있던 그는 연구소 내에서 정보를 찾는 방식에 대해 큰 문제의식을 느끼고 있었다. 당시에 연구소 내에서 정보는 수많은 컴퓨터들에 분산되어 계층적인 폴더 구조에 저장되는 방식이었는데, 이 방식이 __필요한 정보를 찾는데 있어 굉장히 불편 했기 때문__ 이다.
...
비유하자면, CERN 연구소의 모든 정보가 하나의 두꺼운 책으로 쓰여져 있는데, 원하는 정보를 얻기 위해 한 챕터를 읽다가도 참조해야 할 정보가 연달아 나타나 목차 페이지를 살피거나 다른 챕터를 읽는 등 계속 페이지를 왔다갔다 넘겨야 하는 번거로움이 존재한다는 것이다. 무엇보다도 가장 큰 문제는 새로운 정보를 추가하거나 이미 있는 정보를 수정해야 할 경우, 책 자체를 개정해야 하는 것처럼, 시스템 전체를 뜯어고쳐야 할 수도 있다는 점이다. 이러한 불편함으로 인해 팀은 현재 정보 시스템이 실제 사람의 사고하는 방식과 맞지 않기 때문에, 이에 맞는 새로운 정보 시스템이 필요하다고 주장한 것이다.
