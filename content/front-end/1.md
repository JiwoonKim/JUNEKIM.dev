...
- HTML의 공유를 통해 어떠한 변화가 생겼고 이로 인해 '웹'이라는 표현이 어디에서 비롯되었는가?

에 대한 질문들을 자세히 살펴봄으로써, 월드 와이드 웹 (World Wide Web, WWW)이 더 구체적으로는 __하이퍼링크를 통해 전세계의 컴퓨터에 있는 정보를 거미줄과 같이 서로 연결시킨 정보 시스템__ 이라는 사실을 이해해보고자 한다.

#### 왜 하필 HTML을 주고받는가
현재 우리는 웹을 통해 문서, 이미지, 영상 등 다양한 정보를 교환하고 있지만, 초기 웹의 형태는 단순히 HTML이라는 문서만을 교환하는 형태였었다. 다시 말하면, HTML이 바로 웹의 핵심이라고도 할 수 있을 것이다. 자, 그러면 여기서 왜 하필이면 HTML을 사용하여 정보를 교환하였는지를 이해하기 위해서, 조금의 역사적인 상상력을 발휘하여 웹의 아버지인 팀 버너스 리(Tim Berners-Lee)가 당시 어떠한 고민으로 웹을 만들었는지를 살펴보자.
...
#### 정보의 거미줄: 하이퍼텍스트
사실 웹을 제대로 이해하기 위해서는 먼저 웹이 교환하는 주 대상인 하이퍼텍스트란 무엇인지부터 제대로 이해해야 한다. 
...
전문가조차도 자신이 속한 분야의 발전을 따라가지 못할 정도로 정보의 양이 늘어나기 시작했다.
 ...
 양이 폭발적으로 증가. 거기다 기존의 정보 시스템은 책을 읽는 것마냥 순서를 따라야만 정보를 얻을 수 있었기 때문에, 원하는 정보를 찾기에 매우 비효율적이었다. 실제로 책을 읽을 때 필요없는 부분을 건너뛰거나, 더 상세한 설명이 필요하여 맨 뒤의 각주 부분으로 옮겨가거나, 다른 책을 참고하기 위해 읽기를 잠시 멈추거나, 아니면 읽기를 포기하고 다른 책으로 넘어가는 경우가 많지 않은가. 결국, 책과 같은 순차적인 시스템으로 필요한 정보를
