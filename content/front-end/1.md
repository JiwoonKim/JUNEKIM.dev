...
### 월드 와이드 웹
초반에 우리는 웹이 인터넷이라는 통신망을 이용하여 컴퓨터들이 정보를 주고받는 어떤 특정한 서비스라고 설명했었다. 아마 웹에 대해 검색하면, 여기에 살을 조금 붙여서 웹 브라우저 프로그램이 있는 컴퓨터가 서버라는 컴퓨터에게 HTML을 비롯한 다양한 파일 및 데이터를 요청하고 받는 서비스라는 설명이 가장 많이 나올 것이다. 우리는 오늘 여기서 조금 더 나아가, 
- 웹이 수많은 파일 종류 중에서도 왜 하필이면 HTML 파일을 주고받기 시작하였는가?
- HTML의 공유를 통해 어떠한 변화가 생겼고 이로 인해 웹이 어떻게 발전하였는가?
- 웹은 어떻게 한 컴퓨터에서 다른 컴퓨터로 HTML 파일을 주고받는가? (서비스의 작동방식)
...
모든 발명품이 특정한 문제로부터 시작하였듯이, 월드 와이드 웹도 당시 팀이 가지고 있던 어떠한 불만으로부터 출발했다. 당시 CERN이라는 연구소에서 일하고 있던 그는 연구소 내에서 정보를 찾는 방식에 대해 큰 문제의식을 느끼고 있었다. 왜냐하면 당시 연구소 내에서 정보는 수많은 컴퓨터들에 분산되어 계층적인 폴더 구조에 저장되는 방식이었는데, 이 방식이 __필요한 정보를 찾는데 있어 굉장히 불편 했기 때문__ 이다.

<img src="./_images/1_tim_berners_lees_concern.PNG" width=100%>

예를 들자면, A 연구에 대한 정보가 필요하여 한 컴퓨터에서 A 연구 결과를 찾아 읽고 있는데, B라는 용어가 나와 그와 관련된 정보를 또 찾아야 하는 경우를 들 수 있다. 이때 B와 관련된 연구를 찾기 위해 현재 사용중인 컴퓨터의 다른 디렉토리를 찾아봐야하거나, 다른 컴퓨터들 내의 폴더를 또 일일이 뒤져야 하는 수고가 든다. 심지어는 B와 관련된 연구가 하나의 문서에 정리되어 있지 않고 여러 컴퓨터 또는 폴더에 각각 저장되어 있다면, 필요한 부분을 이해하기 위해 수없이 많은 컴퓨터들과 폴더를 일일이 확인해봐야 할 수도 있다. 거기다 만약 B에서 그치지 않고, C 또는 D, 아니면 그 이상의 정보가 연속해서 필요해진다면, 관련 정보를 검색하는데 상당한 시간이 걸리게 된다. 

팀은 CERN 연구소의 이러한 정보 관리 시스템을 하나의 두꺼운 책에 비유하며, 이러한 시스템이 실제 사람의 사고가 흘러가는 과정과 맞지 않기 때문에 새로운 정보 시스템이 필요하다고 주장했다.

실제로 책을 읽을 때 필요없는 부분을 건너뛰거나, 더 상세한 설명이 필요하여 맨 뒤의 각주 부분으로 옮겨가거나, 다른 책을 참고하기 위해 읽기를 잠시 멈추거나, 아니면 읽기를 포기하고 다른 책으로 넘어가는 경우가 많지 않은가. 결국, 책과 같은 순차적인 시스템으로 필요한 정보를

정보를 얻기 위해  각 정보는 특정한 챕터로 넘어가야만 읽을 수 있어 항상 불편하게 왔다갔다하며, 
거기다 업데이트하기가 매우 불편한 구조: 일일이 관련 부분을 찾아야 하며, keeping the book up to date become impractical and the structure of the book needs to be constantly revised

필요한 정보가 있을 때마다 일일이

즉, 팀은 당시 연구소의 정보 체계 시스템이 원하는 정보를 찾기에 비효율적이라고 판단하고 정보를 관리할 수 있는 새로운 시스템의 필요성을 느낀 것이다.

종종 정보가 기록되었음에도 불구하고, 찾을 수 없는 경우가 생겼다

웹이란 참조를 통해 정보를 효과적으로 엮는 하이퍼텍스트 
개인은 자신이 모르는 정보를 다른 사람에게 물어볼 수 있는 방법이 생기게 된 것이다. 
...
#### 정보의 거미줄: 하이퍼텍스트
근대 과학 분야의 발전으로 인해 정보의 양이 폭발적으로 증가하게 되면서 전문가조차 자신의 분야를 모두 아우르지 못하는 

 정보의 양이 전문가조차 아우르지 못하는 수준으로까지 증가하였다.

가 폭발적으로 증가였다.
전문가조차도 자신이 속한 분야의 발전을 따라가지 못할 정도로 정보의 양이 늘어나기 시작했다.

 양이 폭발적으로 증가. 거기다 기존의 정보 시스템은 책을 읽는 것마냥 순서를 따라야만 정보를 얻을 수 있었기 때문에, 원하는 정보를 찾기에 매우 비효율적이었다. 
