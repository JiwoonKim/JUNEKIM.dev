...
2. d[n]과 d[n-1] 등의 관계가 어떻게 연결되는지 잘 살펴보기
    ...
    - ex. 쉬운 계단 수 문제
        - d[n] = 2 * d[n-1] - (n-1) 방식으로 패턴을 만들어나가면 fail: 처음 d[1], d[2], d[3], d[4]까지는 운좋게 맞으나, 그 후부터는 틀림.
        - 1~9를 숫자길이가 하나 짧은 숫자에 붙여나가는 패턴 사용하면 correct: 밑의 문제 풀이 방식 참고

### 백준 문제 풀이
> 눈 여겨 볼 문제들은 링크가 걸려 있음
대표유형:
1. 방법의 수 구하기: d[n]이 d[n-1] 또는 d[n-2]등의 합으로 이루어지는 경우
2. 최대값 & 최소값 구하기: d[n]이 d[n-1] 또는 d[n-2], 그리고 추가적인 입력값 p[n]의 합으로 이루어지는 경우
...
- 풀이: 
    - _각 열마다 경우의 수를 따져보기_ 
        - 스티커를 떼지 않는 경우: d[n][0]
        - 위 칸 스티커를 떼는 경우: d[n][1]
        - 아래 칸 스티커를 떼는 경우: d[n][2]
    - _각 경우마다 가지수를 따져 그 중 최대값_ 이 나오는 경우 찾기 (해당 경우의 가지수 중 최대값 구하기)
        - 스티커를 떼지 않는 경우, 그 전 열에서 스티커를 떼지 않은 경우, 위 칸 스티커를 떼는 경우, 아래 칸 스티커를 떼는 경우가 있음
        - _해당 경우 중 최대값이 나오는 경우가 이 경우의 최대값이다_
