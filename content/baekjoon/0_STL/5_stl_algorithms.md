...
#### Find
시간복잡도: O(N)
- `find(begin, end, value)`: [begin, end)에 포함되어 있는 원소 중에서 첫번째 value의 이터레이터를 반환한다 (찾는 것이 없는 경우에는 end를 반환한다)
```cpp
    vector<int> v = {1, 2, 2, 3, 4, 5};
    auto it = find(v.begin(), v.end(), 2);
    // end()값과 같으면 벡터에 원소가 없음
    if (auto == v.end()) { cout << "찾을 수 없음\n"; }
    else {
        // 인덱스는 begin()값을 빼야지 계산 가능
        cout << (it - v.begin());
    }
```
...
#### Rotate
시간복잡도: O(N)
- `rotate(begin, mid, end)`: [begin, end)를 mid를 기준으로 왼쪽으로 회전시킨다 (a.k.a shift: __mid 값이 begin에 들어가고__, 순서대로 end-1 값까지 들어간다)
```cpp
    vector<int> v = {1, 2, 3, 4, 5};
    int n = v.size();
    for (int i = 0; i < n; i++) {
...
#### Binary_search
- `binary_search(begin, end, value)`: [begin, end) 범위 내에서 value의 값을 __이분탐색__ 하여, 찾으면 true, 못 찾으면 false를 반환한다.
```cpp
    vector<int> v = {1, 2, 5};
    for (int i = 1; i <= 5; i++) {
...
```
- 주의:
    - __정렬된 상태__ 로 사용했을 때 효과가 있다.
    - 찾는 value의 인덱스 값은 알 수 없으며, __단순히 value가 존재하는지 안 하는지의 여부만 알 수 있다__. (위치는 `lower_bound`나 `upper_bound`를 사용하면 된다)
- `binary_search(begin, end, value, cmp)`: [begin, end) 범위 내에서 value의 값을 cmp의 기준으로, 찾으면 true, 못 찾으면 false를 반환한다.

#### Lower_ bound & Upper_ bound
...
- __binary_search를 통해 구현__ 이 되어 있어, 어떤 값의 __위치(by 이터레이터)__ 를 알고 싶을 때 사용한다.
```cpp
...
```
- 정렬된 상태에서 __value의 개수__ 를 찾는데 사용할 수도 있다 (`equal_range`를 사용하면 더 쉽게 한 번에 구할 수 있다!)
```cpp
vector<int> v = {1, 2, 3, 3, 4};
auto l = lower_bound(v.begin(), v.end(), 3); // 3자리 (2 인덱스)
auto r = upper_bound(v.begin(), v.end(), 3); // 4 자리 (4 인덱스)
...
- `min_element(begin, end)`: [begin, end)에서 최소값의 __이터레이터__ 를 구한다.
- `max_element(begin, end)`: [begin, end)에서 최대값의 __이터레이터__ 를 구한다.
```cpp
