...
- __접근__: front와 back, 그리고 [] 또는 iterator를 이용한다.
...
```
- __순회__: 인덱스 (size와 []의 조합) 또는 iterator (begin & end의 조합)을 이용한 다양한 방식이 있다.
```cpp
// 순회접근
...
- __순차 컨테이너를 변형한 자료구조__로 stack, queue, priority_queue가 존재한다.
