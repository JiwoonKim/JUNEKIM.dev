...
```js
window.onscroll = () => {
    // 그전 scroll한 부분 + 현재 창 크기가 전체 페이지의 크기 끝에 다다르면, 화면을 초록색으로 변환
...
... // append post element to posts section
```

### JavaScript Templating
libraries like template literals (__str_replace__: ``${}``)
- since code gets messier using JS to build more complicated user interfaces
    - adding items to the DOM every time (create element, assign class name, set text content ...)
...
- __templates__: more like advanced str_replace (__more of pluggin__)
    - mustache, underscore, handlebar, lowdash, etc.
...
- if template becomes _complicated_, __define template in a separate script__ (defined above the script source of where it is used)
    - after definition, create template based on it in another script
```html
...
...
... // script where template is used 
```
```js
// change only this part from previous example
// create template based on additional script's definition
